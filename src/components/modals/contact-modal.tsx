import { Mail, Clock, Facebook, Instagram, Linkedin } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...formData,
          phone: '', // Optional field for backend compatibility
        }),
      });
        ...
        <div>
          ...
          <div className="space-y-4 mb-8">
            <div className={`flex items-center ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
              ...
              />
              <span className="text-gray-700">info@cactusmediajo.com</span>
            </div>
            <div className={`flex items-center ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
              <Clock
                className={`text-desert-orange w-6 h-6 ${language === 'ar' ? 'ml-3' : 'mr-3'}`}
          ...
          <div>
            ...
            <div className={`flex space-x-4 ${language === 'ar' ? 'space-x-reverse' : ''}`}>
              <a
                href="https://www.facebook.com/cactusmediajo"
                target="_blank"
              ...
              <a
                href="https://www.instagram.com/cactusmediajo"
                target="_blank"
                rel="noopener noreferrer"
                className="bg-cactus-green text-white w-10 h-10 rounded-full flex items-center justify-center hover:opacity-80 transition-opacity"
              >
